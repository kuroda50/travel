...
Future<Map<String, dynamic>?> fetchDocument(String collectionPath, String docId) async {
  try {
    final snapshot = await FirebaseFirestore.instance.collection(collectionPath).doc(docId).get();
...
/// [docId] - ドキュメントID
/// [updates] - 更新内容
Future<void> updateDocument(String collectionPath, String docId, Map<String, dynamic> updates) async {
  try {
    await FirebaseFirestore.instance.collection(collectionPath).doc(docId).update(updates);
  } catch (e) {
    ...
    if (action == "approve" && postId != null) {
      ...
      if (groupChatRoomId != null) {
        ...
        await updateDocument('users', joinUserId, {
          ...
          'chatRooms': FieldValue.arrayUnion([groupChatRoomId]), // groupChatRoomIdを追加
        });
        ScaffoldMessenger.of(context).showSnackBar(
...
/// キックダイアログを表示する関数
void showKickDialog(BuildContext context, String userId, String name, String roomId) {
  showDialog(
    context: context,
    builder: (context) {
      return AlertDialog(
...
Widget buildJoinRequestButton({
  ...
  return FutureBuilder<DocumentSnapshot>(
    ...
    builder: (context, chatRoomSnapshot) {
      if (!chatRoomSnapshot.hasData || chatRoomSnapshot.hasError) return SizedBox(); // データがない場合やエラー時は何も表示しない

      final chatRoomData = chatRoomSnapshot.data!.data() as Map<String, dynamic>;
      if (chatRoomData['recruit'] != true) return SizedBox(); // 募集投稿でない場合は何も表示しない

      final String postId = chatRoomData['postId'] ?? ""; // 投稿IDを取得
      return FutureBuilder<DocumentSnapshot>(
        future: FirebaseFirestore.instance.collection('posts').doc(postId).get(), // 投稿情報を取得
        builder: (context, postSnapshot) {
          if (!postSnapshot.hasData || postSnapshot.hasError) return SizedBox(); // データがない場合やエラー時は何も表示しない

          final postData = postSnapshot.data!.data() as Map<String, dynamic>;
          final String organizerId = postData['organizer']?['organizerId'] ?? ""; // 主催者IDを取得

          if (organizerId == currentUserId) return SizedBox(); // 主催者の場合はボタンを表示しない
...
Widget buildJoinRequestList({
  ...
  return FutureBuilder<DocumentSnapshot>(
    future: chatRoomsCollection.doc(roomId).get(),
    builder: (context, chatRoomSnapshot) {
      if (!chatRoomSnapshot.hasData || chatRoomSnapshot.hasError) return SizedBox();
      ...
      return FutureBuilder<DocumentSnapshot>(
        ...
        builder: (context, postSnapshot) {
          if (!postSnapshot.hasData || postSnapshot.hasError) return SizedBox();

          final postData = postSnapshot.data!.data() as Map<String, dynamic>;
          final String organizerId = postData['organizer']?['organizerId'] ?? "";

          if (currentUserId != organizerId) return SizedBox(); // 主催者でない場合は表示しない
...
void showParticipantDialog(BuildContext context, List<String> participants, String roomId) {
  showDialog(
    context: context,
    builder: (context) {
      return AlertDialog(
                ...
                builder: (context, userSnapshot) {
                  ...
                  return ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () => showKickDialog(context, userId, name, roomId),
                    ),
                  );
                },
              );
