...
class _RecruitmentPostScreenState extends State<RecruitmentPostScreen> {
  ...
  DateTime? picked = await showCustomDatePicker(context, initialTime, label, "未定", selectedStartDate, selectedEndDate);
...
Future<void> _postToFirestore() async {
  ...
  if (!userDoc.exists) {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ユーザー情報が取得できませんでした')));
    return;
  }

  final userData = userDoc.data() as Map<String, dynamic>;
  if (!_validateInputs()) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("すべての必須項目を入力してください"), backgroundColor: Colors.red));
    return;
  }
  ...
  try {
    ...
    context.push('/message-room', extra: {"roomId": roomId, "currentUserId": user.uid});
  } catch (e) {
...
// チャットルームを作成する関数
Future<String> _createChatRoom(String userId, Map<String, dynamic> userData) async {
...
// 投稿データを準備する関数
Map<String, dynamic> _preparePostData(String userId, Map<String, dynamic> userData, String roomId) {
  Map<String, String> genderMap = {'男性': 'male', '女性': 'female', '家族': 'family', 'グループ': 'group'};
  Map<String, String> dayMap = {'月': 'Mon', '火': 'Tue', '水': 'Wed', '木': 'Thu', '金': 'Fri', '土': 'Sat', '日': 'Sun'};
  Map<String, String> paymentMethodMap = {'未定': 'null', '割り勘': 'splitEvenly', '各自自腹': 'eachPays', '主催者が多めに出す': 'hostPaysMore', '主催者が少なめに出す': 'hostPaysLess'};
  ...
  return {
    ...
    "target": {
      "targetGroups": selectedGenderAttributeRecruit.where((gender) => gender != '未定').map((gender) => genderMap[gender]!).toList(),
      "ageMax": selectedAgeRecruit.split('〜')[1] == '未定' ? null : int.parse(selectedAgeRecruit.split('〜')[1]),
      "ageMin": selectedAgeRecruit.split('〜')[0] == '未定' ? null : int.parse(selectedAgeRecruit.split('〜')[0]),
    ...
    "budget": {
      ...
      "budgetMax": selectedBudgetMax.isEmpty ? null : int.parse(selectedBudgetMax),
      "budgetType": selectedPaymentMethod != '未定' ? paymentMethodMap[selectedPaymentMethod] : null,
    },
    "meetingPlace": {
      "region": selectedMeetingRegion,
      "departure": selectedDeparture.isNotEmpty && selectedDeparture[0] != '未定' ? selectedDeparture[0] : null,
    },
